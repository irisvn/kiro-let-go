...
`ClassNetwork` bao gom cac loi: DNS resolution failure, connection timeout, connection refused, TLS handshake failure, host/network unreachable.

Cac loi nay KHONG duoc count vao circuit breaker failure count. Chi trigger retry voi account khac, khong penalize account bi loi network. Dieu nay ngan chan account bi block chi vi van de network tam thoi.

## Auto-Recovery trigger

Khi tat ca accounts bi disable boi circuit breaker (khong con candidate nao available), dispatcher trigger auto-recovery: reset toan bo circuits va bat dau lai selection. Day la co che tu khoi phuc ma khong can admin can thiep.
...
| `ClassNetwork` | Retry voi account khac, KHONG tang failure_count |
